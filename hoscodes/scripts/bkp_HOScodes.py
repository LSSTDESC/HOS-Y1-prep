...
class kappacodes(kappamaps):
    ...
    def run_PDFPeaksMinima(self,map1_smooth,Nmap1,map2_smooth=None,Nmap2=None,is_cross=False):
        ...
        #stddev_kappa_map = 0.017#standard deviation of fiducial kappa maps at the first tomographic bin. if kappa is not sigma_0 normalized, this is an OK value.        
        #bins=np.linspace(-3*stddev_kappa_map,6*stddev_kappa_map,101) 
        #binmids=(bins[1:]+bins[:-1])/2
        bins = np.linspace(-0.02,0.03,221)
        binmids=(bins[1:]+bins[:-1])/2
        ...
        if is_cross:
            ...
            fn_header = self.filenames[Nmap1].split('/')[-1]
            fn_out_counts = self.dir_results+'PDF/'+fn_header.split('.')[0]+'_Nmap%d_Nmap%d_Counts_kmin-0.005_kmax0.005_20kappabins.dat'%(Nmap1+1,Nmap2+1)
            #fn_out_minima = self.dir_results+'peaks/'+fn_header.split('.')[0]+'_Nmap%d_Nmap%d_minima_kmin-0.005_kmax0.005_20kappabins_posRADEC_amp.dat'%(Nmap1+1,Nmap2+1)
            fn_out_peaks = self.dir_results+'peaks/'+fn_header.split('.')[0]+'_Nmap%d_Nmap%d_peaks_kmin-0.005_kmax0.005_20kappabins_posRADEC_amp.dat'%(Nmap1+1,Nmap2+1)
            ...
            fn_header = self.filenames[Nmap1].split('/')[-1]
            #fn_out_counts = self.dir_results+'PDF/'+fn_header.split('.')[0]+'_Nmap%d_Counts_kmin-0.02_kmax0.02_220kappabins.dat'%(Nmap1+1)
            fn_out_counts = self.dir_results+fn_header.split('.')[0]+'_Nmap%d_Counts_kmin-0.02_kmax0.02_220kappabins.dat'%(Nmap1+1)
            #fn_out_minima = self.dir_results+'peaks/'+fn_header.split('.')[0]+'_Nmap%d_minima_kmin-0.005_kmax0.005_20kappabins_posRADEC_amp.dat'%(Nmap1+1)
            #fn_out_peaks = self.dir_results+'peaks/'+fn_header.split('.')[0]+'_Nmap%d_peaks_kmin-0.005_kmax0.005_20kappabins_posRADEC_amp.dat'%(Nmap1+1)

...
sys.path.append('/global/homes/j/jatorres/HOS-Y1-prep/Map3/')
...
from aperture_mass_computer import *
from Peaks_minima import *
from DSS_functions import *
#from ..Map3 import *
#from ..PDF_Peak_Minima import *
#from ..DSS_clean import *

def map2alm(map1,fn_out=None,map2=None,is_cross=False,save_file=False):
    if is_cross:
        C_ells = anafast(map1=map1, map2=map2, nspec=None, 
         lmax=5000, mmax=None, iter=1,
         alm=False,pol=False, use_weights=False,
         datapath=None, gal_cut=0,use_pixel_weights=False)
        C_ells *= 8.0
    else:
        C_ells = anafast(map1, map2=None, nspec=None, 
                     lmax=5000, mmax=None, iter=1,
                     alm=False,pol=False, use_weights=False,
                     datapath=None, gal_cut=0,use_pixel_weights=False)
        C_ells *= 8.0
    ...
    if save_file:
        np.savetxt(fn_out,C_ells)
    
    return C_ells

def map3(theta_aperture_list,map1,nside,fn_out,map2=None,map3=None,is_cross=False):

    if is_cross:

        measureMap3FromKappa_crossbins(map1, map2, map3, thetas=theta_aperture_list, nside=self.nside, verbose=False, doPlots=False, fn_out=fn_out)

    else:
        measureMap3FromKappa(map1, thetas=theta_aperture_list, nside=nside, fn_out=fn_out, verbose=False, doPlots=False)

        
    return None

def CountStatistics(map1,thresholded_bins,fn_out=None,map2=None,is_cross=False,save_file=False):
    """ PDF, Peaks and Minima Counts. It runs in a smoothed map.
    Parameters:
        map1: healpy map
        thresholded_bins: Bins units of the map where the statistic is calculated.
        smoothing_scale: scale length of smoothing in arcmins.

        return:
        vector with bins and counts of the sta
    
    """
    
    binmids=(thresholded_bins[1:]+thresholded_bins[:-1])/2

    if is_cross:
        map1_smooth_ma = hp.ma(map1)
        map2_smooth_ma = hp.ma(map2)
        map1_smooth_ma = map1_smooth_ma+map2_smooth_ma
        counts_smooth,_ = np.histogram(map1_smooth_ma,density=True,bins=thresholded_bins)

        #map2 is added to map1
        ...
        counts_peaks,_ = np.histogram(peak_amp,density=True,bins=thresholded_bins)
        counts_minima,_ = np.histogram(minima_amp,density=True,bins=thresholded_bins)

    else:

        map1_smooth_ma = hp.ma(map1)
        counts_smooth,bins=np.histogram(map1_smooth_ma,density=True,bins=thresholded_bins)
        peak_pos, peak_amp = find_extrema(map1_smooth_ma,lonlat=True)
        #repeat for minima
        minima_pos, minima_amp = find_extrema(map1_smooth_ma,minima=True,lonlat=True)
    ...
    count_stats = np.array([binmids,counts_smooth,counts_peaks,counts_minima]).T

    if save_file:
        np.savetxt(fn_out,count_stats)

    return count_stats

def run_DSS(density_contrast_map,shear_table,pix,gamma1,gamma2,nside,load_prepared_data = False):
...
#Add integrated three-point function
def run_integrated3PCF(tomo_map,tomo_xiA,tomo_xiB,theta_Q_arcmins,theta_T_arcmins,NSIDE,n_g,filepath_map_data,p,is_IA):
    ...
    if is_IA:
        filepath_output = filepath_map_data+'shear+IA_'+p+'_'+tomo_Map+'_'+tomo_xiA+'_'+tomo_xiB+'_All_'+n_g+'/'
    
    else:
        filepath_output = filepath_map_data+'shear_'+p+'_'+tomo_Map+'_'+tomo_xiA+'_'+tomo_xiB+'_All_'+n_g+'/'
    ...
    np.save(filepath_output+p+'_all_patches_M_a_Re.npy', all_patches_M_a_Re)
    np.save(filepath_output+p+'_all_patches_xi_pp_Re.npy', all_patches_xi_pp_Re)
    np.save(filepath_output+p+'_all_patches_xi_mm_Re.npy', all_patches_xi_mm_Re)
    ...
    np.savetxt(filepath_output+p+'_average_over_patches_Re_footprint.dat', dat, delimiter = ' ', header=header)
    ...
    np.savetxt(filepath_output+p+'_stddev_of_mean_over_patches_Re_footprint.dat', dat, delimiter = ' ', header=header)
    ...
    np.savetxt(filepath_output+p+'_cov_over_patches_xi_pp_Re_footprint.dat', xi_pp_Re_cov, delimiter = ' ')
    np.savetxt(filepath_output+p+'_cov_over_patches_xi_mm_Re_footprint.dat', xi_mm_Re_cov, delimiter = ' ')

    np.savetxt(filepath_output+p+'_cov_over_patches_zeta_app_Re_footprint.dat', zeta_app_Re_cov, delimiter = ' ')
    np.savetxt(filepath_output+p+'_cov_over_patches_zeta_amm_Re_footprint.dat', zeta_amm_Re_cov, delimiter = ' ')

    ### Imaginary parts

    np.save(filepath_output+p+'_all_patches_M_a_Im.npy', all_patches_M_a_Im)
    np.save(filepath_output+p+'_all_patches_xi_pp_Im.npy', all_patches_xi_pp_Im)
    np.save(filepath_output+p+'_all_patches_xi_mm_Im.npy', all_patches_xi_mm_Im)
    ...
    np.savetxt(filepath_output+p+'_average_over_patches_Im_footprint.dat', dat, delimiter = ' ', header=header)
    ...
    np.savetxt(filepath_output+p+'_stddev_of_mean_over_patches_Im_footprint.dat', dat, delimiter = ' ', header=header)
    ...
    np.savetxt(filepath_output+p+'_cov_over_patches_xi_pp_Im_footprint.dat', xi_pp_Im_cov, delimiter = ' ')
    np.savetxt(filepath_output+p+'_cov_over_patches_xi_mm_Im_footprint.dat', xi_mm_Im_cov, delimiter = ' ')

    np.savetxt(filepath_output+p+'_cov_over_patches_zeta_app_Im_footprint.dat', zeta_app_Im_cov, delimiter = ' ')
    np.savetxt(filepath_output+p+'_cov_over_patches_zeta_amm_Im_footprint.dat', zeta_amm_Im_cov, delimiter = ' ')

    ### angular separations

    np.savetxt(filepath_output+p+'_angular_separations_arcmin.dat', theta.T)
